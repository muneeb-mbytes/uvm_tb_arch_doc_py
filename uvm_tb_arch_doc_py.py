...
def top(): # drawing top level structure
    ...
    for i in range(intfs):
        ...
        bottom_right = 100 + (
                    (i + 1) * (w - 200) / intfs) - 10, h - 160 # co-ordinate decider '''(i+1)*(w-100)/intfs'''
        ...
        tb.text((100 + (i * (w - 200) / intfs) + 20, h - 200), "Interface " + str(i + 1), fill="black",
                font=font) # avoid taking default text coordinates'''
    # DUT
...
def agent(a, i, name):
    ...
    if (a > 5 & a <= 9): # if number of agents are greater than 5 draw sub-components small
        label = name # "AGT"+str(i+1)
        tb.rectangle([x + (x1 - x) / 2, y + 30, (x1 - 40 / a), int(y1 - 180)], "white", "black",
                     5) # default values for all instance
        tb.rectangle([x + 40 / a, y + 170, -10 + x + (x1 - x) / 2, int(y1 - 40)], "white", "black",
                     5) # default values for all instance
        ...
        tb.polygon(((x + 10 + 40 / a - 10, h - 238), (x + 10 + 40 / a + 10, h - 238), (x + 10 + 40 / a, h - 218)),
                   fill="white", outline="black") # port connection
        tb.line((((x + 10 + (x1 - x) / 2), (y1 - 40)), ((x + (x1 - x) / 2 + 10), h - 218)), "black",
                5) # VIF to monitor
        tb.polygon(((x + (x1 - x) / 2, y1 - 20), (x + 20 + (x1 - x) / 2, y1 - 20), (x + 10 + (x1 - x) / 2, y1 - 40)),
                   fill="white", outline="black") # port connection
        tb.line((((x1 - 40 / a), (y1 - 100)), ((x1 + 50), y1 - 100)), "black", 5) # monitor to scoreboard
        tb.polygon(
            (x1 - 40 / a + 20, y1 - 100, x1 - 40 / a + 10, y1 - 90, x1 - 40 / a, y1 - 100, x1 - 40 / a + 10, y1 - 110),
            fill="white", outline="black") # analysis connection
        ...
        tb.rectangle((x + 10 + 40 / a - 10, y + 170, x + 10 + 40 / a + 10, y + 150), fill="white",
                     outline="black") # port connection
        tb.line((((x + 10 + +40 / a), (y + 100)), (((x + (x1 - x) / 2)), y + 100)), "black", 5) # sequencer to driver
        tb.ellipse((x + (x1 - x) / 2 - 20, y + 90, x + (x1 - x) / 2, y + 110), fill="white",
                   outline="black") # export connection
        # agent label
        ...
        tb.polygon(((x - 10 + (x1 - x) / 2, h - 238), (x + 10 + (x1 - x) / 2, h - 238), (x + (x1 - x) / 2, h - 218)),
                   fill="white", outline="black") # port connection
        tb.polygon(((x - 10 + (x1 - x) / 2, y1 + 20), (x + 10 + (x1 - x) / 2, y1 + 20), (x + (x1 - x) / 2, y1)),
        ...
        img1.show()
        path_filename = (r"C:\Python_new\agents" + str(
            i + 1) + ".jpg") # create agents folder before running code
        print(path_filename)
        ...
        mark = x + 10
        block(top_left=(x + (x1 - x) / 2, y + 30), bottom_right=(int(x1 - 40 / a), int(y1 - 180)), label=" ",
              color_fill="white") # "grey") #for sequencer
        block(top_left=(x + 40 / a, y + 170), bottom_right=(-10 + x + (x1 - x) / 2, y1 - 40), label="",
              color_fill="white") # "violet") #for driver
        block(top_left=(x + (x1 - x) / 2, y + 170), bottom_right=(int(x1 - 40 / a), int(y1 - 40)), label="",
              color_fill="white") # "skyblue") #for monitor
    ...
    for root, dirs, files in os.walk(root_dir, onerror=None): # to loop inside all files of directory
        for filename in files:
            ...
            with open(file_path, "rb") as f: # read file as binary
                for line in f:
                    line = line.decode("ISO-8859-1 ") # decode to string for read
...
def tb_comps(): # generic word in tb components that can be searched to determine presence of comp in TB
    #component_search("topology")
    component_search("uvm_test_top")
    component_search("   env")
    component_search("AS_AXI_Agent_")
    component_search("magna_scoreboard")
    #component_search("uvm_test_top")
...
def component_name_finder(keyword, line):
    ...
    if keyword + "_1" in lookup: # if keyword already exists in lookup
        ...
        if (keyword == "AS_AXI_Agent"): # make it first instance of agent
            key = keyword + "_1"
            lookup[key] = name
...
def read_file_draw():
    ...
    file = f.read()
    # print (file)
    if (re.search("uvm_test_top", file)): # find keyword in file
        top() # draw top
        test(lookup['uvm_test_top']) # draw test with found name
        env(lookup['   env']) # draw env with found name
        scoreboard(lookup['magna_scoreboard']) # draw scb with found name
        a = file.count("AS_AXI_Agent_") # find number of agents
        a= int(input ("number of Agents:"))
        '''#a = 6 #uncomment and add values for agents explicitly
        ...
        for i in range(a):
            ...
            agent(a,i,"AS_AXI_Agent_"+str(i+1)) # draw agent with agent names'''
    f.close()
...
img.save(r"C:\Users\Priya A\PycharmProjects\UVM_ARCH_TB\log\tb_arch.jpg")
image = cv2.imread(r"C:\Users\Priya A\PycharmProjects\UVM_ARCH_TB\log\tb_arch.jpg")
intfs = int(input("enter the number of interfaces"))
v = 0.1;
for i in range(intfs):
    print("value of v", v)
    g = w * v;
    image = cv2.arrowedLine(image, (int(g), h - 160), (int(g), h - 120), (000), 2)
    v = v + (1/(intfs * 1.2));
print("value of v1", v)
cv2.imshow('abc', image)
cv2.imwrite(r"C:\Users\Priya A\PycharmProjects\UVM_ARCH_TB\log\tb_arch.jpg", image)
cv2.waitKey(0)
